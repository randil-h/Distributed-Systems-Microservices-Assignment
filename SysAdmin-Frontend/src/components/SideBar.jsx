...
export default function SideBar() {
    ...
    return (
        <div className="bg-gray-900 bottom-0 top-12 fixed w-1/6 border-r border-gray-700 flex flex-col justify-between divide-y divide-gray-700">
            {/* First ul */}
            <ul className="flex flex-col items-center text-gray-300 font-medium text-base py-4 px-3">
                {menuItems.map((item) => (
                    item.disabled ? (
                        <div key={item.name} className="w-full flex flex-row cursor-not-allowed opacity-50">
                            <li
                                className="flex flex-row w-full h-12 my-1 text-gray-400 px-1"
                            >
                                <div className="flex items-center justify-between w-full">
                                    <div className="pl-3 flex items-center">
                                        {item.icon && <item.icon className="mr-4 h-5 w-5 text-gray-500" />}
                                        {item.name}
                                    </div>
                                    <div className="flex items-center">
                                <span className="bg-yellow-500 text-black text-xs px-1 py-0.5 rounded ml-2">
                                    Coming Soon
                                </span>
                                    </div>
                                </div>
                        ...
                        <Link key={item.name} to={item.path} className="w-full flex flex-row">
                            <li
                                className={`flex flex-row w-full h-12 my-1 focus:outline-none focus:ring focus:ring-lime-400 transition-all duration-200 px-1 ${
                                    isActive(item.path) ? "bg-gray-800 text-lime-400 rounded-xl px-3 shadow-lg" : "hover:bg-gray-800 hover:rounded-xl hover:shadow-lg"
                                }`}
                            >
                                <div className="flex items-center justify-between w-full">
                                    <div className="pl-3 flex items-center">
                                    ...
                                    {isActive(item.path) && item.count && (
                                        <span
                                            className="bg-red-600 rounded-full w-5 h-5 mr-2 flex items-center justify-center text-xs text-gray-100">
                                    {item.count}
                                </span>
                                    )}
                                </div>
                    ...
                    <Link key={item.name} to={item.path} className="w-full">
                        ...
                        >
                            <div className="flex items-center justify-between w-full">
                                ...
                                {isActive(item.path) && item.count && (
                                    <span
                                        className="bg-gray-700 rounded-full w-5 h-5 mr-2 flex items-center justify-center text-xs text-gray-200">
                                {item.count}
                            </span>
                                )}
                            </div>
